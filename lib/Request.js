    ...
    run() {
        ...
        return new Promise((resolve, reject) => {
            axios_1.default({
                ...
                const r = new Response_1.Response(resp.status, resp.statusText, resp.headers, resp.data);
                this.response = r;
                resolve(r);
            })
                .catch((err) => {
                ...
                this.endTime = Date.now();
                if (err.response) {
                    const r = new Response_1.Response(err.response.status, err.response.statusText, err.response.headers, err.response.data);
                    this.response = r;
                }
                reject(err);
            });
